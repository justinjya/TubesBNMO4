    ...
    for i in range(length(x)):
        if id == x[i][0]:
            ...
            if tambah < 0 and abs(tambah) > x[i][5]: # Validasi bahwa hasil akhir stok tidak bisa kurang dari 0
                print('Stok game',x[i][1],'gagal dikurangi karena stok kurang. Stok sekarang:',x[i][5], '(<',abs(tambah),')')
            else:
                # Menambah stok
                x[i][5] = str(int(x[i][5]) + tambah)
                if tambah > 0:
                    print('Stok game', x[i][1],'berhasil ditambahkan. Stok sekarang:',x[i][5])
                else:
                    print('Stok game', x[i][1],'berhasil dikurangi. Stok sekarang:',x[i][5])
            break
